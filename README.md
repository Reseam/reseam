...
| `reseam-model` | Shared data for Rust and generated clients |
| `reseam-sdk` | Application service used directly by the CLI |
| `reseam-sdk-native` | Typed BoltFFI application bindings |
...
Desktop JNI packaging also requires a host `clang` on `PATH`. BoltFFI builds the desktop library for the current host; Linux is verified in this migration.
...
This builds the `reseam` CLI and native SDK. The checked-in Kotlin bindings and the SDK's Android `jniLibs/*.so` files are regenerated by `cargo xtask regen all`, which also packages the desktop JNI library; run it whenever you change a `#[export]` Rust function:
...
- `docs/bindings.md` records the BoltFFI architecture and migration decisions.
