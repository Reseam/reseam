...
Windows builds the CLI and the engine with the MSVC toolchain. `cargo xtask jni-host`, which links the desktop JNI shim for the Manager, runs on Linux only.
...
This builds the `reseam` CLI plus the embedded patcher and SDK shim. The checked-in Kotlin bindings and the SDK's Android `jniLibs/*.so` files are regenerated by `cargo xtask regen all`, which runs the BoltFFI codegen; run it whenever you change a `#[export]` Rust function:
