...
sealed interface ReseamSetting<T> {
    val key: String
    val title: String
    val summary: String?
    val default: T
...
data class SettingsSection(
    val title: String,
    val settings: List<ReseamSetting<*>>,
)

interface SettingsHost {
    val appId: String
    val extensionDex: List<String>
    val schemaPath: String get() = "assets/reseam/settings.json"
...
fun settingsHostPatch(
    ...
    return object : ReseamPatch {
        ...
        override fun afterDependents(ctx: PatchRuntime) {
            val schema = ReseamSettingsRegistry.schema(hostKey = name, appId = host.appId)
            ctx.files.write(host.schemaPath, schema.toByteArray(Charsets.UTF_8))
            host.install(ctx)
            ReseamSettingsRegistry.clear(name)
        }
    }
}
...
internal fun registerPatchSettings(
    ...
    ReseamSettingsRegistry.register(hostKey, patchName, settings)
}

private object ReseamSettingsRegistry {
...
private fun StringBuilder.appendSetting(setting: ReseamSetting<*>) {
    append("{\"type\":")
    appendJson(
        when (setting) {
