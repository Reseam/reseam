...
| `ExternalPatch(bundle, id)` | A patch in another bundle, for `dependsOn`. Generated by the build from the bundles a module declares under `reseam { bundle(...) }`; see [Depending on another bundle](4_patches.md#depending-on-another-bundle). |
...
Patch display names need not be unique. Each patch's ID derives from its public declaration (`package.property`), including named patches. Renaming the property or its package changes its ID; changing the display name does not. The engine refers to a patch as `<bundle>/<id>`, its reference, with the bundle name from `manifest.toml`; dependencies, results, progress events, and logs carry references. CLI selection and options accept a reference, an ID that is unique across the loaded bundles, or a display name that resolves unambiguously for the target package; `bundle list` prints the IDs. SDK metadata exposes `bundle`, `id`, and `name`.

Each result carries `patch` (the reference), `hidden`, and `required_by`, so a client can report a run without re-deriving it. `required_by` lists the running patches that pulled this one in and is empty when the user asked for it directly, which is what separates "7 patches applied" from the dependencies that came with them. Count user-facing work as the results where `hidden` is false and `required_by` is empty.
