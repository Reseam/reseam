...
**Bundle manifest**: `manifest.toml` inside the bundle. Holds the bundle name, author, description, format version, the engine version that packed it, and the hash of every payload file. Signed by the bundle author's Ed25519 key.
...
**`patches.json`**: the release index for a bundle. Lists the bundle's identity, public key, and every release. Read by the Reseam API and Reseam Manager. Written or updated by `reseam publish patches`. `manager.json` is the same shape for Reseam Manager releases, written by `reseam publish manager`.
