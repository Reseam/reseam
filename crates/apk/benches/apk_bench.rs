...
fn default_opts() -> ParseOptions { ParseOptions::default() }

fn skip_verify_opts() -> ParseOptions {
    ParseOptions { skip_checksum: true, skip_signature: true, ..ParseOptions::default() }
}
...
fn extract_dex_files_from_apk(apk_path: &str) -> Vec<(String, Vec<u8>)> {
    ...
    let mut names: Vec<String> = (0..archive.len())
        .filter_map(|i| {
            ...
            if name.ends_with(".dex") { Some(name) } else { None }
        })
        .collect();
    names.sort();
    for name in &names {
...
fn bench_parse(c: &mut Criterion) {
    if !std::path::Path::new(YOUTUBE_APK).exists() { return; }
    ...
    group.bench_with_input(BenchmarkId::new("parse_skip_verify", name), buf, |b, buf| {
        b.iter(|| stitch_dex::parse(buf, skip_verify_opts()).unwrap());
    });
    let total_size: usize = dex_files.iter().map(|(_, b)| b.len()).sum();
    group.bench_function(BenchmarkId::new("all_dex_files", format!("{}MB", total_size / (1024 * 1024))), |b| {
        b.iter(|| { for (_, buf) in &dex_files { stitch_dex::parse(buf, skip_verify_opts()).unwrap(); } });
    });
    group.finish();
}
...
fn bench_write(c: &mut Criterion) {
    ...
    let parsed: Vec<_> = dex_files.iter()
        .map(|(name, buf)| (name.clone(), stitch_dex::parse(buf, default_opts()).unwrap()))
        .collect();
...
fn bench_search(c: &mut Criterion) {
    ...
    group.bench_function("find_method_by_name", |b| {
        b.iter(|| dex.find_method_by(|method_id, _class, _em| dex.string(method_id.name) == "onCreate"));
    });
    group.bench_function("find_all_init_methods", |b| {
        b.iter(|| dex.find_methods_by(|method_id, _class, _em| dex.string(method_id.name) == "<init>"));
    });
    group.bench_function("find_class", |b| {
        b.iter(|| dex.find_class("Ljava/lang/Object;"));
    });
    group.bench_function("find_methods_with_opcodes", |b| {
        let pattern = [InstructionPattern::Opcode(OpcodeMatcher::Const), InstructionPattern::Opcode(OpcodeMatcher::Return)];
        b.iter(|| dex.find_methods_with_opcodes(&pattern));
    });
    group.bench_function("find_methods_wildcard_pattern", |b| {
        let pattern = [InstructionPattern::Opcode(OpcodeMatcher::InvokeVirtual), InstructionPattern::Any, InstructionPattern::Opcode(OpcodeMatcher::ReturnObject)];
        b.iter(|| dex.find_methods_with_opcodes(&pattern));
    });
    group.finish();
}
...
fn bench_mutation(c: &mut Criterion) {
    ...
    group.bench_function("intern_string_new", |b| {
        b.iter_batched(|| dex.clone(), |mut dex| { dex.intern_string("Lcom/benchmark/NewClass;"); }, criterion::BatchSize::SmallInput);
    });
    group.bench_function("intern_string_existing", |b| {
        b.iter_batched(|| dex.clone(), |mut dex| { dex.intern_string("Ljava/lang/Object;"); }, criterion::BatchSize::SmallInput);
    });
    group.bench_function("intern_method", |b| {
        b.iter_batched(|| dex.clone(), |mut dex| {
            dex.intern_method("Lcom/benchmark/Test;", "doSomething", "(II)V").expect("valid");
        }, criterion::BatchSize::SmallInput);
    });
    group.bench_function("return_early", |b| {
        b.iter_batched(
            || { let mut d = dex.clone(); for class in &mut d.classes { if let Some(ref mut data) = class.class_data { for m in data.direct_methods.iter_mut() { if m.code.is_some() { return d; } } } } d },
            |mut dex| { for class in &mut dex.classes { if let Some(ref mut data) = class.class_data { for m in data.direct_methods.iter_mut() { if let Some(ref mut code) = m.code { code.return_early(); return; } } } } },
            criterion::BatchSize::SmallInput,
        );
    });
    group.finish();
...
fn bench_instagram(c: &mut Criterion) {
    if !std::path::Path::new(INSTAGRAM_APK).exists() { return; }
    ...
    group.bench_with_input(
        BenchmarkId::new("parse_largest", format!("{} ({}MB)", largest_name, largest_buf.len() / (1024 * 1024))),
        largest_buf, |b, buf| { b.iter(|| stitch_dex::parse(buf, skip_verify_opts()).unwrap()); },
    );
    group.bench_function(BenchmarkId::new("parse_all", format!("{}MB", total_size / (1024 * 1024))), |b| {
        b.iter(|| { for (_, buf) in &dex_files { stitch_dex::parse(buf, skip_verify_opts()).unwrap(); } });
    });
    ...
    let buffers: Vec<&[u8]> = dex_files.iter().map(|(_, b)| b.as_slice()).collect();
    group.bench_function(BenchmarkId::new("multi_dex_parse", format!("{} files", dex_files.len())), |b| {
        b.iter(|| stitch_dex::MultiDexContainer::parse(&buffers, skip_verify_opts()).unwrap());
    });
    let container = stitch_dex::MultiDexContainer::parse(&buffers, default_opts()).unwrap();
    ...
    group.bench_function(BenchmarkId::new("lazy_parse_largest", largest_name), |b| {
        b.iter(|| {
            stitch_dex::parse(largest_buf, ParseOptions { lazy: true, skip_checksum: true, skip_signature: true, ..ParseOptions::default() }).unwrap()
        });
    });
    group.finish();
}

criterion_group!(benches, bench_parse, bench_write, bench_round_trip, bench_search, bench_mutation, bench_multi_dex, bench_instagram);
