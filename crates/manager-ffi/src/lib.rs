...
use boltffi::export;
use reseam_library::{
    built_in_trust_store, inspect_apk, inspect_with_trust, measure_patch, PatchArtifact,
    PatchMetrics, PatchOutput, PatchRequest, PatchSelection, RunEvent, TrustStore,
};
...
pub fn inspect_apk_json(apk_path: String, split_paths_json: String) -> Result<String, String> {
    ...
    let metadata = inspect_apk(PathBuf::from(apk_path).as_path(), &split_paths)
