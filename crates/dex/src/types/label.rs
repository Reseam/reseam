...
mod tests {
    ...
    #[test]
    fn simple_goto_forward() -> std::result::Result<(), String> {
        ...
        assert!(
            matches!(&code.instructions[0], Instruction::Goto32 { offset } if *offset == 4),
            "expected Goto32, got {:?}",
            code.instructions[0]
        );
        Ok(())
    ...
    #[test]
    fn simple_goto_backward() -> std::result::Result<(), String> {
        let mut b = CodeBuilder::new(1, 0, 0);
        ...
        let code = b.build().map_err(|e| format!("build failed: {e}"))?;
        assert!(
            matches!(&code.instructions[1], Instruction::Goto32 { offset } if *offset == -1),
            "expected Goto32, got {:?}",
            code.instructions[1]
        );
        Ok(())
    ...
    #[test]
    fn if_branch_with_label() -> std::result::Result<(), String> {
        ...
        let code = b.build().map_err(|e| format!("build failed: {e}"))?;
        assert!(
            matches!(&code.instructions[0], Instruction::IfEqz { offset, .. } if *offset == 3),
            "expected IfEqz, got {:?}",
            code.instructions[0]
        );
        Ok(())
    ...
    #[test]
    fn try_catch_with_labels() -> std::result::Result<(), String> {
        let mut b = CodeBuilder::new(2, 0, 0);
    ...
    fn unbound_label_returns_error() {
        ...
        let result = b.build();
        assert!(result.is_err(), "expected Err from build");
        if let Err(e) = result {
            assert!(e.to_string().contains("unbound label"));
        }
    }
}
