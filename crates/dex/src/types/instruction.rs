...
pub enum Instruction {
    ...
    FilledNewArray { type_: TypeIdx, args: SmallVec<[u8; 5]> },
    FilledNewArrayRange {
        type_: TypeIdx,
        first_reg: u16,
        count: u8,
    ...
    CmpLFloat { dest: u8, a: u8, b: u8 },
    CmpGFloat { dest: u8, a: u8, b: u8 },
    CmpLDouble { dest: u8, a: u8, b: u8 },
    CmpGDouble { dest: u8, a: u8, b: u8 },
    ...
    InvokeVirtual { method: MethodIdx, args: SmallVec<[u8; 5]> },
    InvokeSuper { method: MethodIdx, args: SmallVec<[u8; 5]> },
    InvokeDirect { method: MethodIdx, args: SmallVec<[u8; 5]> },
    InvokeStatic { method: MethodIdx, args: SmallVec<[u8; 5]> },
    InvokeInterface { method: MethodIdx, args: SmallVec<[u8; 5]> },
    InvokeVirtualRange {
        method: MethodIdx,
        first_reg: u16,
        count: u8,
    ...
    RsubIntLit16 { dest: U4, src: U4, literal: i16 },
    ...
    SparseSwitchPayload { keys_and_targets: Vec<(i32, i32)> },
    FillArrayDataPayload { element_width: u16, data: Vec<u8> },
    RawInstruction { code_units: SmallVec<[u16; 5]> },
}
...
impl Instruction {
    ...
    pub fn code_units(&self) -> u32 {
        match self {
            ...
            Self::ConstWide { .. } => 5,
            Self::PackedSwitchPayload { targets, .. } => {
                (1 + 1 + 2 + targets.len() * 2) as u32
            }
            Self::SparseSwitchPayload { keys_and_targets, .. } => {
    ...
    pub fn dest_register(&self) -> Option<u16> {
        match self {
            ...
            Self::Move16 { dest, .. } | Self::MoveWide16 { dest, .. } | Self::MoveObject16 { dest, .. } => {
                Some(*dest)
            }
