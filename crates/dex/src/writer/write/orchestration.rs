use super::DexWriter;
use super::{annotations, classdata, code, finalize, types};
...
impl DexWriterWriteExt for DexWriter {
    fn write_dex(&mut self, dex: &DexFile) -> Result<()> {
        ...
        if !dex.method_handles.is_empty() {
            ...
            for mh in &dex.method_handles {
                ...
                let member_id = match &mh.member {
                    crate::model::method_handle::MethodHandleMember::Field(f) => f.0 as u16,
                    crate::model::method_handle::MethodHandleMember::Method(m) => m.0 as u16,
                };
        ...
        let (proto_param_offsets, class_interface_offsets) = types::write_type_lists(self, dex);

        let class_ann_datas = annotations::write_annotations(self, dex);

        let code_methods = crate::writer::write::methods_with_code(dex);
        code::write_code_items(self, &code_methods);

        classdata::write_class_data_items(self, dex);
        ...
        let (static_values_offsets, call_site_data_offsets) =
            types::write_encoded_arrays(self, dex);

        if let Some(ref hidden_api) = dex.hidden_api {
            self.align(4);
            let hidden_api_off = self.pos();
            types::write_hidden_api(self, hidden_api, dex);
            self.map_entries.push(MapItem {
                type_code: crate::model::map::TYPE_HIDDENAPI_CLASS_DATA_ITEM,
                size: 1,
