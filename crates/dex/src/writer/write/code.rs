use super::super::code_writer::encode_instructions;
use super::DexWriter;
use crate::encoding::leb128::{write_sleb128, write_uleb128};

/// Writes code items, deduplicated debug info, and code-item backpatches.
pub(crate) fn write_code_items(w: &mut DexWriter, methods: &[&crate::model::class::EncodedMethod]) {
    ...
    for method in methods {
        let code = method
            .code
            .as_ref()
            .expect("methods_with_code returned a method without code");
        w.align(4);
        let off = w.pos();
    ...
    if code_item_count > 0 {
        w.map_entries.push(crate::model::map::MapItem {
            type_code: crate::model::map::TYPE_CODE_ITEM,
    ...
    for method in methods {
        ...
        if let Some(ref debug) = code.debug_info {
            let mut tmp = Vec::new();
            super::super::debug_writer::write_debug_info(&mut tmp, debug);
    ...
    if debug_count > 0 {
        w.map_entries.push(crate::model::map::MapItem {
            type_code: crate::model::map::TYPE_DEBUG_INFO_ITEM,
            size: debug_count,
            offset: debug_start,
        });
    }
    ...
    {
        let mut di_idx = 0;
        for (ci_idx, method) in methods.iter().enumerate() {
            let code = method
                .code
                .as_ref()
                .expect("methods_with_code returned a method without code");
...
fn write_code_item(w: &mut DexWriter, code: &crate::model::code::CodeItem) {
