use super::encoded_value_reader::read_encoded_annotation;
...
fn read_annotation_item(buf: &[u8], off: u32) -> Result<AnnotationItem> {
    ...
    let visibility_byte = buf[pos];
    let visibility = AnnotationVisibility::from_u8(visibility_byte).ok_or(
        DexError::InvalidAnnotationVisibility {
            value: visibility_byte,
        },
    )?;
