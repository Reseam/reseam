...
impl CodeItem {
    ...
    /// Adjust all branch offsets, try items, and catch handlers after
    /// an insertion or removal at `addr` with the given `delta` (positive = inserted, negative = removed).
    fn fixup_offsets(&mut self, addr: u32, delta: i32) {
        // Fix branch offsets in instructions
        ...
        for t in &mut self.tries {
            ...
            if t.start_addr >= addr {
                ...
                // Insertion/removal inside the try block — adjust count
                t.insn_count = (t.insn_count as i32 + delta) as u16;
            }
        }

        // Fix catch handler addresses
...
/// Adjust branch offset in a single instruction.
/// `insn_addr` is the code unit address of this instruction.
/// `change_addr` is where the insert/remove happened.
/// `delta` is the size change (positive = inserted, negative = removed).
fn fixup_branch(insn: &mut Instruction, insn_addr: u32, change_addr: u32, delta: i32) {
    match insn {
        ...
        // Switch payload targets are relative to the switch instruction, not this payload.
        // They get fixed up when the switch instruction itself is processed.
        Instruction::PackedSwitchPayload { .. } | Instruction::SparseSwitchPayload { .. } => {
            // Payload targets are relative to the switch instruction, not this payload.
            // They are re-encoded by the writer from the IR.
        }
        _ => {}
    }
}

/// Compute adjusted offset. If the branch crosses the change point, adjust by delta.
fn fixup_i32(offset: i32, insn_addr: u32, change_addr: u32, delta: i32) -> i32 {
    ...
    if offset > 0 {
        // Forward branch: adjust if change point is between insn and target
        ...
        // Backward branch: adjust if change point is between target and insn
