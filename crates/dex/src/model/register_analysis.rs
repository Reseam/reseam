...
fn live_registers(code: &CodeItem, at_index: usize) -> HashSet<u16> {
    ...
    // Backward scan: registers written before at_index may be live
    ...
    // Forward scan: registers read before being written are definitely live
...
fn written_registers(insn: &Instruction) -> Vec<u16> {
    match insn {
        ...
        // 2addr forms: dest_a is both read and written
        ...
        // CheckCast modifies the ref in-place conceptually, but doesn't write a new value
        Instruction::Nop
...
fn read_registers(insn: &Instruction) -> Vec<u16> {
    match insn {
        ...
        // 2addr: dest_a is read, b is read
